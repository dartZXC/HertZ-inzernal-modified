...
uint32_t HashString(const char *str, int32_t len=0); //if 0, stops on null, like for a string
...
void SetIntWithTarget(int32_t *p_out_dest, int r_target, int r_amount);
int mod(int a, int b);
/**
...
std::string DataToByteHexDisplay(uint8_t *pData, int maxNumCharsToShow);
std::string IntToMoneyBillions(int bil, int n);
std::string IntToTime(uint32_t ms, bool bTextFormat = false); //converts 18000 (ms) to 3:00, suitable for a game "time left" display.  if bTextformat, it would say "3 min" instead.
std::string IntToTimeSeconds(uint32_t seconds, bool bTextFormat = false , bool showDays = false); //converts 180 (seconds) to 3:00, suitable for a game "time left" display.  if bTextformat, it would say "3 min" instead.
