...
{
	f -= 1.0f;
	int32_t changeX = (int32_t)(float(GetWidth())*f) / 2;
	int32_t changeY = (int32_t)(float(GetHeight())*f) / 2;
...
void rtRect32::AdjustPosition(int32_t x, int32_t y)
{
	left += x;
	right += x;
	top += y;
...
void rtRect32::Inflate(int32_t x, int32_t y)
{
	left -= x;
	right += x;
	top -= y;
