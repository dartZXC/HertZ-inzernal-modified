...
uint32_t HashString(const char *str, int32_t len)
{
	if (!str) return 0;
...
string DataToByteHexDisplay(uint8_t *pData, int maxNumCharsToShow)
{
...
string PopFirstParmString(string *lineInOut, const string delimiter)
{
	...
	*lineInOut = "";
	...
	for (uint32_t i=1; i < pieces.size(); i++)
	{
		if ( !lineInOut->empty())
		{
			*lineInOut += delimiter;
...
string IntToTimeSeconds(uint32_t seconds, bool bTextFormat, bool showDays) //converts 180 (seconds) to 3:00, suitable for a game "time left" display.  if bTextformat, it would say "3 min" instead.
{
...
string IntToTime(uint32_t ms, bool bTextFormat)
{
